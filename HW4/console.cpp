...
private:
    ...
    char reply_input[max_length]={0};
    char input[max_length]={0};

public:
    client(int ID,tcp::socket socket):ID(ID),socket_(move(socket)),resolver(io_context),query(request[ID].host, request[ID].port)
    {
...
private:
    void Resolve(){
        ...
        resolver.async_resolve(query,[this,self](boost::system::error_code ec,tcp::resolver::iterator iter) {
                if (!ec) {
                    endpoint = *iter;
                    SendSocks4package();
                }
    ...
    void SendSocks4package(){
        auto self(shared_from_this());
        string ip = endpoint.address().to_string();
        vector<string> buff = SplitString(ip,".");
        // unsigned short dstport = endpoint.port();
        unsigned char reply[9] = {
            4, 1, (unsigned char)(endpoint.port() / 256), (unsigned char)(endpoint.port() % 256), (unsigned char)stoi(buff[0]), (unsigned char)stoi(buff[1]), (unsigned char)stoi(buff[2]), (unsigned char)stoi(buff[3]), 0
        };
        async_write(socket_,boost::asio::buffer(reply,9),
    ...
    void ReadSocks4Package(){
        auto self(shared_from_this());
        socket_.async_read_some(boost::asio::buffer(reply_input, max_length),
            [this, self](boost::system::error_code ec, std::size_t length) {
                if(!ec){
    ...
    void do_read(){
        auto self(shared_from_this());
        socket_.async_read_some(boost::asio::buffer(input, max_length),[this, self](boost::system::error_code ec, std::size_t length)
        {
            if(!ec){
                // cerr<<"do_read"<<endl;
                string Msg = input;
    ...
    void do_write(){
        ...
        boost::asio::async_write(socket_,boost::asio::buffer(line.c_str(), line.length()),[this, self](boost::system::error_code ec, std::size_t /*length*/)
        {
            if(!ec) {
                do_read();
            }
...
private:
    ...
    void do_proxy_resolve(){
        auto self(shared_from_this());
        resolver.async_resolve(proxy_host,proxy_port,[this, self](const boost::system::error_code ec, tcp::resolver::iterator iter){
                if(!ec) {
                    endpoint = *iter;
    ...
    void do_connect(){
        ...
        socket.async_connect(endpoint,[this,self](boost::system::error_code ec){
            ...
            if(!ec){                
                ...
                make_shared<client>(ID,move(socket))->start();
            }
            else{
...
int main(){
    ...
    HTML();
    cerr<<proxy_host<<":"<<proxy_port<<endl;
    try{
        for(int i = 0; i < 5; i++){
...
void parse_envirnoment(string QUERY_STRING){
    ...
    while(index != std::string::npos){
        ...
        if(buf[0]=='h'){
            ...
            proxy_happen = true;
            proxy_host = buf.substr(3);
        }
        else if(buf[0]=='s'&&buf[1]=='p'){
            proxy_port = buf.substr(3);
        }
        front = index + 1;
        index = QUERY_STRING.find('&',front);
    }
